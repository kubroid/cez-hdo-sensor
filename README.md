...
- 📡 **Поддержка сигналов**: a3b4dp01, a3b4dp02, a3b4dp06, error (защитный режим)
...
- `error_mode` - Активен ли защитный режим (только при ошибках)
- `error_message` - Сообщение об ошибке (если есть)
- `safety_mode` - Описание защитного режима (при ошибках)
...
- `error` - защитный режим (всегда низкий тариф)
...
### Когда использовать сигнал "error"?
Используйте сигнал `error` в следующих случаях:
- Нестабильное интернет-соединение
- Частые проблемы с CEZ API
- Нужна максимальная надежность (всегда низкий тариф)
- Тестирование системы автоматизации
- Временное решение при проблемах с обычными сигналами
...
- **error**: Защитный сигнал - всегда активирует низкий тариф при любых проблемах

Выберите сигнал, который соответствует вашему договору с поставщиком электроэнергии.

### 🛡️ Защитный режим (Error Signal)

Сигнал `error` является специальным защитным механизмом:
- **Всегда возвращает низкий тариф** для максимальной безопасности
- Автоматически активируется при любых проблемах:
  - Нет интернет-соединения
  - Ошибки сервера ČEZ API
  - Проблемы с парсингом данных
  - Любые другие ошибки
- **Использование**: Выберите этот сигнал, если хотите гарантированно использовать низкий тариф
- **Атрибуты**: В режиме error добавляются атрибуты `error_mode`, `error_message` и `safety_mode`
- **Безопасность**: Даже при сбоях интеграции ваши устройства будут работать на дешевом тарифе
