...
### Когда проявляется автоматическая обработка ошибок?
Система автоматически переходит в безопасный режим в следующих случаях:
- Нестабильное интернет-соединение
- Технические работы или недоступность CEZ API
- Ошибки парсинга данных расписания
- Любые другие непредвиденные проблемы
- В безопасном режиме всегда активируется низкий тариф
...
Интеграция имеет встроенные механизмы безопасности:
- **Автоматическая обработка ошибок**: При любых проблемах (нет интернета, ошибки API, проблемы с парсингом) система автоматически переключается в безопасный режим
- **Низкий тариф по умолчанию**: В случае ошибок всегда активируется низкий тариф для защиты от высоких счетов
- **Видимость ошибок**: Все ошибки отображаются в атрибутах сенсора (`error_mode`, `error_message`, `safety_mode`)
- **Постоянная доступность**: Сенсор остается доступным даже при ошибках
